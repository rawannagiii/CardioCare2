...
import {
  ...
  Check, ChevronRight, CircleCheck, CircleUserRound, ClipboardCheck, Clock3,
  Droplets, Dumbbell, FileText, HeartPulse, House, Info, Lightbulb, Pill, Plus,
  Scale, Share2, ShieldCheck, Sparkles, Stethoscope, Timer, TrendingDown,
...
type Medication = { id: number; name: string; dose: string; frequency: string; scheduledTime: string; status: 'Taken' | 'Missed' | 'Upcoming' };
...
type HealthGoal = { id: string; label: string; description: string; progress: number; target: string; checked: boolean };
...
const initialMedications: Medication[] = [
  { id: 1, name: 'Lisinopril', dose: '10 mg', frequency: 'Once each morning', scheduledTime: '08:00', status: 'Taken' },
  { id: 2, name: 'Metoprolol succinate', dose: '25 mg', frequency: 'Once each evening', scheduledTime: '20:00', status: 'Upcoming' },
  { id: 3, name: 'Furosemide', dose: '20 mg', frequency: 'Once each morning', scheduledTime: '08:00', status: 'Taken' },
];
...
const initialGoals: HealthGoal[] = [
  { id: 'bp-checks', label: 'Regular blood pressure checks', description: 'Log readings on the days your care plan calls for.', progress: 72, target: '3 of 4 this week', checked: false },
  { id: 'medication-consistency', label: 'Medication consistency', description: 'Mark each prescribed medicine after your routine.', progress: 67, target: '4 of 6 doses', checked: false },
  { id: 'movement', label: 'Physical activity', description: 'Build toward movement that feels safe and sustainable.', progress: 80, target: '24 of 30 min', checked: false },
  { id: 'sodium', label: 'Sodium reduction', description: 'Notice labels and choose lower-sodium options when possible.', progress: 74, target: '1,480 of 2,000 mg guide', checked: false },
  { id: 'healthy-weight', label: 'Maintaining healthy weight', description: 'Track trends without judging a single day.', progress: 58, target: '168.4 lb trend', checked: false },
];
const educationCards: EducationCard[] = [
  ...
  { title: 'A practical path toward quitting smoking', summary: 'Support can make a hard change more manageable.', body: 'If you smoke, talk with your clinician about a quit plan, medicines, counseling, or a quitline. Notice triggers without judgment and celebrate each smoke-free day. A slip is information, not failure; return to your plan and ask for support.', category: 'Smoking cessation' },
  { title: 'Make space for a calmer nervous system', summary: 'Small pauses can support the rest of your care routine.', body: 'Try a short walk, paced breathing, a quiet stretch, or writing down one concern before bed. Stress is real, but it is not a personal failure. If worry, low mood, or sleep problems persist, bring them to your care team.', category: 'Stress management' },
];
...
const navItems = [
  ...
  { href: '/goals', label: 'Goals', icon: ClipboardCheck },
...
function readingsInRange(readings: BloodPressureReading[], range: 'week' | 'month') {
  const newest = latest(readings);
  if (!newest) return readings;
  const anchor = +new Date(newest.recordedAt);
  const days = range === 'week' ? 7 : 31;
  return readings.filter((reading) => anchor - +new Date(reading.recordedAt) <= days * 24 * 60 * 60 * 1000);
}
function readingGuidance(reading: BloodPressureReading) {
  if (reading.systolic >= 180 || reading.diastolic >= 120) return 'This is a notably high reading. Rest quietly and repeat it. If it remains this high or you have concerning symptoms, seek urgent medical guidance.';
  if (reading.systolic >= 130 || reading.diastolic >= 80) return 'This reading is a little above the general educational reference range. Trends and context matter; bring repeated patterns to your care team.';
  return 'This reading sits within a commonly referenced educational range. One reading is only a snapshot, so keep looking at patterns over time.';
}
...
function HomePage({ readings, medications, lifestyle, goals }: { readings: BloodPressureReading[]; medications: Medication[]; lifestyle: LifestyleMetrics; goals: HealthGoal[] }) {
  ...
  const goalProgress = Math.round(goals.reduce((sum, goal) => sum + goal.progress, 0) / goals.length);
  return <>
    ...
    <div className="cc-content">
      <div className="cc-grid cc-dashboard-grid">
        ...
        <StatCard label="Medication" value={`${adherence}`} unit="%" trend={`${taken} of ${medications.length} taken`} icon={Pill} />
         <StatCard label="Heart rate" value={`${bp.heartRate}`} unit="bpm" trend="At latest check-in" icon={HeartPulse} />
         <StatCard label="Weight" value={`${lifestyle.weight}`} unit="lb" trend="Personal trend" icon={Scale} warm />
         <div className="cc-card cc-card-pad cc-span-2 cc-quick-actions"><div className="cc-section-head"><div><span className="cc-eyebrow">Quick actions</span><h2>Keep the next step close.</h2></div><Zap size={18} color="hsl(var(--accent))" /></div><div className="cc-action-row"><Link href="/blood-pressure" className="cc-button cc-button-soft" data-testid="button-quick-record-bp"><Activity size={15} /> Record BP</Link><Link href="/medications" className="cc-button cc-button-soft" data-testid="button-quick-medications"><Pill size={15} /> Check medications</Link><Link href="/doctor-summary" className="cc-button cc-button-ghost" data-testid="button-quick-doctor-summary"><FileText size={15} /> Review summary</Link></div></div>
         <div className="cc-card cc-card-pad cc-span-2"><div className="cc-section-head"><div><span className="cc-eyebrow">Today's medications</span><h2>One less thing to hold in your head.</h2></div><Link className="cc-link" href="/medications" data-testid="link-home-medications">See all</Link></div><div className="cc-list">{medications.map((med) => <div className="cc-list-row" key={med.id} data-testid={`row-home-medication-${med.id}`}><div className="cc-row-main"><span className="cc-icon-box"><Pill size={16} /></span><span><strong>{med.name}</strong><small>{med.dose} · {med.frequency} · {med.scheduledTime}</small></span></div><span className={`cc-status ${med.status.toLowerCase()}`}>{med.status}</span></div>)}</div></div>
        ...
        <div className="cc-card cc-card-pad cc-span-2"><div className="cc-section-head"><div><span className="cc-eyebrow">Recent signal</span><h2>Blood pressure at a glance</h2></div><Link className="cc-link" href="/blood-pressure" data-testid="link-home-bp-history">View history</Link></div><MiniChart readings={readings} /></div>
         <div className="cc-card cc-card-pad"><div className="cc-section-head"><div><span className="cc-eyebrow">Health goals</span><h2>Small wins, visible.</h2></div><Link className="cc-link" href="/goals" data-testid="link-home-goals">See goals</Link></div><div className="cc-stat-number">{checkedGoals}<small> of {goals.length} checked</small></div><div className="cc-progress-track"><div className="cc-progress-fill coral" style={{ width: `${goalProgress}%` }} /></div><div className="cc-progress-copy"><span>Overall progress</span><span>{goalProgress}%</span></div></div>
...
function BloodPressurePage({ readings, onAdd }: { readings: BloodPressureReading[]; onAdd: (reading: BloodPressureReading) => void }) {
  ...
  const bp = latest(readings);
  const visibleReadings = readingsInRange(readings, range);
  return <>
    ...
    <div className="cc-content">
       <div className="cc-grid cc-bp-grid">
         <div className="cc-card cc-card-pad"><div className="cc-section-head"><div><span className="cc-eyebrow">{range === 'week' ? 'Last 7 days' : 'Last 30 days'}</span><h2>Your trend, not a verdict.</h2></div><div className="cc-pill-tabs"><button className={`cc-pill-tab ${tab === 'chart' ? 'active' : ''}`} onClick={() => setTab('chart')} data-testid="button-bp-chart-tab">Chart</button><button className={`cc-pill-tab ${tab === 'history' ? 'active' : ''}`} onClick={() => setTab('history')} data-testid="button-bp-history-tab">History</button></div></div><div className="cc-range-tabs" role="group" aria-label="Blood pressure time range"><button className={range === 'week' ? 'active' : ''} onClick={() => setRange('week')} data-testid="button-bp-range-week">Weekly</button><button className={range === 'month' ? 'active' : ''} onClick={() => setRange('month')} data-testid="button-bp-range-month">Monthly</button></div>{tab === 'chart' ? <MiniChart readings={visibleReadings} /> : <div className="cc-table-wrap"><table className="cc-table"><thead><tr><th>Date</th><th>Reading</th><th>Heart rate</th><th>Note</th></tr></thead><tbody>{visibleReadings.map((reading) => <tr key={reading.id} data-testid={`row-bp-reading-${reading.id}`}><td><strong>{formatDate(reading.recordedAt)}</strong><br /><span style={{ color: 'hsl(var(--muted-foreground))' }}>{formatTime(reading.recordedAt)}</span></td><td className="cc-mono"><strong>{reading.systolic}/{reading.diastolic}</strong> <span style={{ color: 'hsl(var(--muted-foreground))' }}>mmHg</span></td><td className="cc-mono">{reading.heartRate} bpm</td><td>{reading.note || '—'}</td></tr>)}</tbody></table></div>}</div>
         <div className="cc-card cc-card-pad"><span className="cc-eyebrow">Latest reading</span><div className="cc-stat-number" style={{ fontSize: 38, marginTop: 24 }}>{bp.systolic}<small>/ {bp.diastolic} mmHg</small></div><p className="cc-subtitle" style={{ marginTop: 10 }}>Recorded {formatDate(bp.recordedAt)} at {formatTime(bp.recordedAt)}</p><div className="cc-progress-track"><div className="cc-progress-fill" style={{ width: `${Math.min(bp.systolic / 180 * 100, 100)}%` }} /></div><div className="cc-progress-copy"><span>Educational context</span><span>{bp.heartRate} bpm</span></div><div className="cc-guidance"><Info size={16} /><p><strong>What this may mean</strong>{readingGuidance(bp)}</p></div></div>
      </div>
      <EmergencyNotice />
...
function GoalsPage({ goals, onToggle }: { goals: HealthGoal[]; onToggle: (id: string) => void }) {
  const average = Math.round(goals.reduce((sum, goal) => sum + goal.progress, 0) / goals.length);
  return <>
    <PageHeader eyebrow="Your care plan" title="Cardiovascular health goals" subtitle="A gentle checklist for the routines you want to keep visible. Your care team can help make these goals personal." />
    <div className="cc-content">
      <div className="cc-goals-intro cc-card cc-card-pad"><div><span className="cc-eyebrow">This week</span><h2>Progress is information, not a grade.</h2><p className="cc-subtitle">Monitor patterns that matter to you, then bring questions or changes to your clinician.</p></div><div className="cc-goal-score"><strong>{average}%</strong><span>overall guide</span></div></div>
      <div className="cc-goals-list">{goals.map((goal) => <article className={`cc-card cc-goal-card ${goal.checked ? 'checked' : ''}`} key={goal.id} data-testid={`card-goal-${goal.id}`}><button className="cc-goal-toggle" onClick={() => onToggle(goal.id)} aria-pressed={goal.checked} data-testid={`button-toggle-goal-${goal.id}`}>{goal.checked ? <CircleCheck size={22} /> : <span />}</button><div className="cc-goal-copy"><div className="cc-section-head"><div><span className="cc-eyebrow">Personal goal</span><h3>{goal.label}</h3></div><strong>{goal.progress}%</strong></div><p>{goal.description}</p><div className="cc-progress-track"><div className="cc-progress-fill" style={{ width: `${goal.progress}%` }} /></div><div className="cc-progress-copy"><span>{goal.checked ? 'Checked for today' : 'In progress'}</span><span>{goal.target}</span></div></div></article>)}</div>
      <EmergencyNotice />
    </div>
  </>;
}

function DoctorSummaryPage({ readings, medications, lifestyle }: { readings: BloodPressureReading[]; medications: Medication[]; lifestyle: LifestyleMetrics }) {
  const [showSummary, setShowSummary] = useState(false);
  const [symptoms, setSymptoms] = useState('No symptoms reported today.');
  const bp = latest(readings);
  const prior = readings[1] || bp;
  const systolicTrend = bp.systolic - prior.systolic;
  return <>
    <PageHeader eyebrow="Appointment prep" title="Doctor summary" subtitle="A concise local preview to help you organize the conversation with your healthcare provider." action={<button className="cc-button cc-button-primary" onClick={() => setShowSummary(true)} data-testid="button-open-provider-summary"><Share2 size={16} /> Share with healthcare provider</button>} />
    <div className="cc-content">
      <div className="cc-summary-banner cc-card cc-card-pad"><div><span className="cc-eyebrow">Review before you go</span><h2>Bring the pattern, not just one number.</h2><p className="cc-subtitle">This preview combines your recent readings, routine notes, and the symptoms you choose to report. It stays on this device.</p></div><FileText size={34} color="hsl(var(--primary))" /></div>
      <div className="cc-summary-grid">
        <div className="cc-card cc-card-pad"><span className="cc-eyebrow">Recent blood pressure</span><div className="cc-stat-number">{bp.systolic}/{bp.diastolic}<small> mmHg</small></div><p className="cc-subtitle">{systolicTrend === 0 ? 'Similar to the prior reading' : `${Math.abs(systolicTrend)} mmHg ${systolicTrend > 0 ? 'higher' : 'lower'} systolic than the prior reading`}</p></div>
        <div className="cc-card cc-card-pad"><span className="cc-eyebrow">Medication adherence</span><div className="cc-stat-number">{Math.round(medications.filter((med) => med.status === 'Taken').length / Math.max(medications.length, 1) * 100)}<small>% today</small></div><p className="cc-subtitle">{medications.filter((med) => med.status === 'Taken').length} of {medications.length} marked taken</p></div>
        <div className="cc-card cc-card-pad"><span className="cc-eyebrow">Heart rate</span><div className="cc-stat-number">{bp.heartRate}<small> bpm</small></div><p className="cc-subtitle">At {formatDate(bp.recordedAt)} · {formatTime(bp.recordedAt)}</p></div>
        <div className="cc-card cc-card-pad"><span className="cc-eyebrow">Lifestyle snapshot</span><div className="cc-summary-facts"><span><strong>{lifestyle.steps.toLocaleString()}</strong> steps</span><span><strong>{lifestyle.exerciseMinutes}</strong> min movement</span><span><strong>{lifestyle.weight}</strong> lb</span><span><strong>{lifestyle.sleepHours}</strong> hr sleep</span><span><strong>{lifestyle.sodiumMg.toLocaleString()}</strong> mg sodium</span></div></div>
      </div>
      <div className="cc-card cc-card-pad cc-symptom-card"><div className="cc-section-head"><div><span className="cc-eyebrow">Reported symptoms</span><h2>What should your provider know?</h2></div><CalendarDays size={19} color="hsl(var(--accent))" /></div><textarea className="cc-input" value={symptoms} onChange={(event) => setSymptoms(event.target.value)} data-testid="input-reported-symptoms" aria-label="Reported symptoms" /><p className="cc-field-note">If you are having severe chest pain, severe difficulty breathing, fainting, sudden weakness, or another emergency warning sign, seek urgent care now instead of waiting for an appointment.</p></div>
      <div className="cc-privacy-note"><ShieldCheck size={17} /><span><strong>Privacy and safety</strong>This is a local, review-ready prototype. It does not send, share, diagnose, or replace professional medical advice.</span></div>
      {showSummary && <ProviderSummaryModal readings={readings} medications={medications} lifestyle={lifestyle} symptoms={symptoms} onClose={() => setShowSummary(false)} />}
      <EmergencyNotice />
    </div>
  </>;
}

function ProviderSummaryModal({ readings, medications, lifestyle, symptoms, onClose }: { readings: BloodPressureReading[]; medications: Medication[]; lifestyle: LifestyleMetrics; symptoms: string; onClose: () => void }) {
  const bp = latest(readings);
  const prior = readings[1] || bp;
  const adherence = Math.round(medications.filter((med) => med.status === 'Taken').length / Math.max(medications.length, 1) * 100);
  return <div className="cc-modal-backdrop"><div className="cc-modal cc-summary-modal" role="dialog" aria-modal="true" aria-labelledby="provider-summary-title"><div className="cc-modal-header"><div><span className="cc-eyebrow">Local preview</span><h2 id="provider-summary-title">Share with healthcare provider</h2><p className="cc-subtitle">Prepared June 18, 2024 · CardioCare sample record</p></div><button className="cc-close" onClick={onClose} aria-label="Close provider summary" data-testid="button-close-provider-summary"><X size={16} /></button></div><div className="cc-summary-document"><div className="cc-summary-document-head"><HeartPulse size={20} /><strong>CardioCare health review</strong></div><div className="cc-summary-document-grid"><div><span>Latest BP</span><strong>{bp.systolic}/{bp.diastolic} mmHg</strong><small>{formatDate(bp.recordedAt)} at {formatTime(bp.recordedAt)}</small></div><div><span>BP trend</span><strong>{bp.systolic - prior.systolic >= 0 ? '+' : ''}{bp.systolic - prior.systolic} systolic</strong><small>Compared with prior entry</small></div><div><span>Heart rate</span><strong>{bp.heartRate} bpm</strong><small>At latest check-in</small></div><div><span>Medication adherence</span><strong>{adherence}%</strong><small>{medications.filter((med) => med.status === 'Taken').length} of {medications.length} marked taken today</small></div></div><div className="cc-summary-section"><span>Recent readings</span><p>{readings.slice(0, 4).map((reading) => `${reading.systolic}/${reading.diastolic} on ${formatDate(reading.recordedAt)}`).join(' · ')}</p></div><div className="cc-summary-section"><span>Lifestyle</span><p>{lifestyle.steps.toLocaleString()} steps · {lifestyle.exerciseMinutes} minutes movement · {lifestyle.weight} lb · {lifestyle.sleepHours} hours sleep · {lifestyle.sodiumMg.toLocaleString()} mg sodium · {lifestyle.waterGlasses} glasses water</p></div><div className="cc-summary-section"><span>Reported symptoms</span><p>{symptoms || 'No symptoms reported.'}</p></div></div><div className="cc-privacy-note"><ShieldCheck size={15} /><span>Privacy note: this is a local preview only. Nothing is transmitted, and this summary is not diagnostic.</span></div><div className="cc-form-actions"><button className="cc-button cc-button-ghost" onClick={onClose} data-testid="button-cancel-provider-summary">Close preview</button><button className="cc-button cc-button-primary" onClick={onClose} data-testid="button-finish-provider-summary"><Check size={15} />Done reviewing</button></div></div></div>;
}
...
function ProfilePage() {
  return <>
    <PageHeader eyebrow="Your care circle" title="Profile" subtitle="Keep the details here current so your care team and trusted contact have the right context." action={<button className="cc-button cc-button-ghost" onClick={() => window.alert('Profile editing is available in this prototype.')} data-testid="button-edit-profile"><UserRound size={15} /><span>Edit profile</span></button>} />
     <div className="cc-content"><div className="cc-profile-grid"><div className="cc-card cc-profile-hero"><span className="cc-avatar">{initials(patient.name)}</span><h2>{patient.name}</h2><p>Age {patient.age} · Living with {patient.conditions.join(' and ').toLowerCase()}</p><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 22 }}>{patient.conditions.map((condition) => <span className="cc-time-chip" key={condition}>{condition}</span>)}</div></div><div className="cc-card cc-card-pad"><div className="cc-section-head"><div><span className="cc-eyebrow">Patient details</span><h2>At a glance</h2></div><Stethoscope size={20} color="hsl(var(--primary))" /></div><div className="cc-detail-list"><div className="cc-detail"><span>Full name</span><strong data-testid="text-profile-name">{patient.name}</strong></div><div className="cc-detail"><span>Age</span><strong data-testid="text-profile-age">{patient.age} years</strong></div><div className="cc-detail"><span>Primary clinician</span><strong data-testid="text-profile-doctor">{patient.doctor}</strong></div><div className="cc-detail"><span>Emergency contact</span><strong data-testid="text-profile-emergency">{patient.emergencyContact}</strong></div></div></div><div className="cc-card cc-card-pad cc-span-2"><div className="cc-section-head"><div><span className="cc-eyebrow">About CardioCare</span><h2>A companion for the in-between moments.</h2></div><HeartPulse size={20} color="hsl(var(--accent))" /></div><p className="cc-subtitle">CardioCare is a digital-health prototype designed to support self-management and treatment engagement among patients with hypertension and cardiovascular disease. It is a portfolio concept for organizing conversations, routines, and personal observations.</p><div className="cc-about-grid"><span><strong>Self-management</strong>Make everyday routines easier to notice.</span><span><strong>Care conversations</strong>Bring clearer context to appointments.</span><span><strong>Education</strong>Explore plain-language, non-diagnostic guidance.</span></div><div className="cc-alert" style={{ marginTop: 20 }}><ShieldCheck size={18} /><div><strong>Privacy and safety</strong>This prototype keeps sample information in local React state on this device. It is not clinically validated, diagnostically accurate, or proven effective, and it does not replace professional medical advice.</div></div></div><div className="cc-card cc-card-pad cc-span-2"><div className="cc-section-head"><div><span className="cc-eyebrow">Your care team</span><h2>Questions are welcome here.</h2></div><HeartPulse size={20} color="hsl(var(--accent))" /></div><p className="cc-subtitle">Bring your readings, medication questions, and anything that has changed in your routine to your next appointment. You never need to figure it out alone.</p></div><div className="cc-span-2"><EmergencyNotice /></div></div></div>
  </>;
}
...
function Router({ readings, onAddReading, medications, onUpdateMedication, onAddMedication, lifestyle, onUpdateLifestyle, goals, onToggleGoal }: { readings: BloodPressureReading[]; onAddReading: (reading: BloodPressureReading) => void; medications: Medication[]; onUpdateMedication: (id: number, status: Medication['status']) => void; onAddMedication: (medication: Medication) => void; lifestyle: LifestyleMetrics; onUpdateLifestyle: (metrics: LifestyleMetrics) => void; goals: HealthGoal[]; onToggleGoal: (id: string) => void }) {
  return <AppShell><Switch><Route path="/"><HomePage readings={readings} medications={medications} lifestyle={lifestyle} goals={goals} /></Route><Route path="/blood-pressure"><BloodPressurePage readings={readings} onAdd={onAddReading} /></Route><Route path="/medications"><MedicationsPage medications={medications} onUpdate={onUpdateMedication} onAdd={onAddMedication} /></Route><Route path="/goals"><GoalsPage goals={goals} onToggle={onToggleGoal} /></Route><Route path="/lifestyle"><LifestylePage lifestyle={lifestyle} onUpdate={onUpdateLifestyle} /></Route><Route path="/education"><EducationPage /></Route><Route path="/doctor-summary"><DoctorSummaryPage readings={readings} medications={medications} lifestyle={lifestyle} /></Route><Route path="/profile"><ProfilePage /></Route><Route component={NotFoundPage} /></Switch></AppShell>;
}
